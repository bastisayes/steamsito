...
$script:langs = @{
    "es" = @{ activar="Activar +300";activarSub="Activa mas de 300 juegos"
        ...
        detenerTodo="Detener procesos + eliminar luas";detenerTodoSub="Mata procesos y borra todos los luas"
        histBorrado="Historial borrado";histBorradoMsg="Se eliminaron todos los codigos activos del registro."
        procDetenido="Procesos detenidos";procDetenidoMsg="Se detuvieron los procesos en segundo plano y se eliminaron todos los luas."
    ...
    "en" = @{ activar="Activate +300";activarSub="Activate over 300 games"
        ...
        detenerTodo="Stop processes + remove luas";detenerTodoSub="Kill processes and delete all luas"
        histBorrado="History cleared";histBorradoMsg="All active codes have been removed from the registry."
        procDetenido="Processes stopped";procDetenidoMsg="Background processes stopped and all luas deleted."
    ...
    "pt" = @{ activar="Ativar +300";activarSub="Ative mais de 300 jogos"
        ...
        detenerTodo="Parar processos + remover luas";detenerTodoSub="Mata processos e deleta todos os luas"
        histBorrado="Historico limpo";histBorradoMsg="Todos os codigos ativos foram removidos do registro."
        procDetenido="Processos parados";procDetenidoMsg="Processos em segundo plano parados e todos os luas deletados."
...
function Refresh-AllText{
    ...
    $script:cConfig.Tag.Title=T "config";$script:cConfig.Tag.Sub=T "configSub";$script:cConfig.Invalidate()
...
# Configuracion (reemplaza Pagina Web)
$script:cConfig=New-Card -X $PAD -Y $WEB_Y -W $CW -H $FCH -Title (T "config") -Sub (T "configSub") -Icon "webpage" -Click {Switch-ToConfig}
$script:mp.Controls.Add($script:cConfig)
...
# Watcher toggle
$script:sWatcher=New-CfgBtn $sY (T "watcherOn") "Click para desactivar" {$null}
$script:sWatcher.Add_Click({
    $script:watcherEnabled = -not $script:watcherEnabled
    if ($script:watcherEnabled) {
        # Launch watcher
        try {
        ...
        } catch { Write-ErrorLog "Toggle watcher on" $_ }
    } else {
        # Kill watcher
...
# Detener procesos + eliminar luas
$script:sKill=New-CfgBtn ($sY+120) (T "detenerTodo") (T "detenerTodoSub") {
    if ([System.Windows.Forms.MessageBox]::Show("SE DETENDRAN todos los procesos en segundo plano y se ELIMINARAN todos los luas.`nContinuar?","Detener todo","YesNo","Warning") -ne "Yes") { return }
    # Kill watcher
    if ($script:watcherProcess -and -not $script:watcherProcess.HasExited) { try { $script:watcherProcess.Kill(); $script:watcherProcess.WaitForExit(2000) } catch {} }
    $script:watcherProcess = $null
    # Kill background fix jobs
    ...
    # Remove luas from all steam libraries
    $steamRoot = Get-SteamPath
    if ($steamRoot) {
    ...
    foreach ($lb in $libs) {
        $stp = Join-Path $lb "steamapps\common"
        if (Test-Path $stp) {
    ...
    # Clear timers
    Save-Timers @(); $script:activeCodes.Clear(); Refresh-Codes
    [System.Windows.Forms.MessageBox]::Show((T "procDetenidoMsg"),(T "procDetenido"),"OK","Information")
}
